...
static void add_to_removed_stack(struct lf_list *list, struct lf_list_node *node_to_add)
{
	...
	do {
		...
		pr_debug("add_to_removed_stack: Current removed_head is %p\n", (void*)old_head_val);

		if ((struct lf_list_node *)old_head_val == node_to_add) {
			pr_warn("add_to_removed_stack: node %p is already head of removed_stack. Cycle prevented. NOT ADDING.\n",
					node_to_add);
			return;
		}
	...
	pr_debug("add_to_removed_stack: Successfully added node %p. New head %p, its next %p\n",
			 node_to_add, (void*)new_head_ptr_val, node_to_add->removed_link);
	return;
}
...
void lf_list_free(struct lf_list *list, struct kmem_cache *lsbdd_node_cache, struct kmem_cache *lsbdd_value_cache)
{
	...
	pr_debug("lf_list_free: Starting main list traversal from node %p\n", node);
	...
	pr_debug("lf_list_free: Finished main list traversal.\n");
	...
	pr_debug("lf_list_free: Finished freeing nodes from removed stack.\n");


	if (list->head) {
		pr_debug("lf_list_free: Freeing head node %p\n", list->head);
		if (list->head != last_freed_node_addr) { // Check before freeing head
			...
			pr_warn("lf_list_free: Head node %p already freed. Skipping.\n", list->head);
		}
		list->head = NULL;
	}
	if (list->tail) {
		pr_debug("lf_list_free: Freeing tail node %p\n", list->tail);
		if (list->tail != last_freed_node_addr) { // Check before freeing tail
			...
			pr_warn("lf_list_free: Tail node %p already freed. Skipping.\n", list->tail);
		}
		list->tail = NULL;
	}

	pr_debug("lf_list_free: Freeing linked list structure %p\n", list);
...
retry_search_outer:
    ...
    while (HAS_MARK(t_next) || (t != list->tail && t->key < key)) { 
        ...
        t_next = t->next;
        pr_debug("lf_list_lookup: Inner loop: Advanced t=%p (key %llu), t_next=%p\n",
                  t, t->key, t_next);
    ...
    pr_debug("lf_list_lookup: Inner loop finished. right_node=%p (key %llu). Final left_node=%p, left_next_snap=%p\n",
             right_node, right_node != list->tail ? right_node->key : (sector_t)-1, *left_node_out, left_node_next_snap);

    // Corruption checks

    if (left_node_next_snap == right_node) {
        pr_debug("lf_list_lookup: Window appears clean (left_next_snap == right_node).\n");
        // Check if the right_node itself is marked for deletion
        if (right_node != list->tail && HAS_MARK(right_node->next)) {
             pr_debug("lf_list_lookup: right_node %p (key %llu) is marked for deletion. Retrying outer search.\n",
                      right_node, right_node->key);
            // According to reference logic, we should retry the search if the target is marked.
            goto retry_search_outer;
        }
        pr_debug("lf_list_lookup: Success. Returning right_node %p (key %llu).\n",
                 right_node, right_node != list->tail ? right_node->key : (sector_t)-1);
        return right_node;
    }
    /** Window is dirty.
	 ...
	 */
        ...
        if (unlikely((*left_node_out == list->head) && (right_node == list->head))) {
            pr_err("lf_list_lookup[Ref]: !!! SAFETY ABORT: Attempted CAS would set head->next = head! ...\n");
            cpu_relax();
        }
        if (unlikely(*left_node_out == right_node && *left_node_out != list->head)) { // Check for node->next = node
             pr_err("lf_list_lookup[Ref]: !!! SAFETY ABORT: Attempted CAS would set node->next = node! ...\n");
             cpu_relax();
         }
        pr_debug("lf_list_lookup: Window dirty (left_next_snap %p != right_node %p). Attempting cleanup CAS.\n",
                 left_node_next_snap, right_node);
        ...
        if (SYNC_LCAS(&((*left_node_out)->next), left_node_next_snap, right_node) == left_node_next_snap) {
            pr_debug("lf_list_lookup: Cleanup CAS success! List modified. Retrying outer search.\n");
            goto retry_search_outer;
        } else {
            ...
            pr_debug("lf_list_lookup: Cleanup CAS failed (concurrent modification). Retrying outer search.\n");
            goto retry_search_outer;
        }
    }

    pr_err("lf_list_lookup: Reached end of function unexpectedly for key %llu.\n", key);
    return NULL;
}
