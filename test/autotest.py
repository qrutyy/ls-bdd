...
class File:
    ...
    def compare(self, other_file):
        ...
        print(
            "\033[1m"
            + f"Files {self.path} and {other_file.path} are identical."
            + "\033[0m"
        )
        return 0
...
class Test:
    ...
    def process_file(self, input_file, output_file):
        ...
        block_sizes = (
            ...
            else self.get_even_divisors(file_size // 1024)
        )
        print(block_sizes)
        if self.mode == "seq":
            seek_offset = 0
            ...
            for write_bs, read_bs in prod:
                print()
                print(
                    "\033[1m" + f"=== OPERATION N.{self.test_count + 1} ===" + "\033[0m"
                )
                ...
                self.run_dd_command(
                    f"dd if={input_file.path} of=/dev/{self.vbd_name} oflag=direct bs={write_bs}k count={write_count} seek={seek_offset // write_bs // 1024} skip={seek_offset // write_bs // 1024}"
                )
                self.run_dd_command(
                    f"dd if=/dev/{self.vbd_name} of={output_file.path} iflag=direct bs={read_bs}k count={read_count} skip={skip_offset // read_bs // 1024} seek={skip_offset // read_bs // 1024}"
                )
            ...
            for write_bs, read_bs in product(block_sizes, repeat=2):
                ...
                self.run_dd_command(
                    f"dd if={input_file.path} of=/dev/{self.vbd_name} oflag=direct bs={write_bs}k count={write_count}"
                )
                self.run_dd_command(
                    f"dd if=/dev/{self.vbd_name} of={output_file.path} iflag=direct bs={read_bs}k count={read_count}"
                )
                self.test_count += 1
            ...
            print(
                f"Mode {self.mode} isn't supported. Check -h for information about available parameters."
            )
...
class TestManager:
    ...
    def run(self):
        if self.args.clear:
            ...
            file_size_kb = (
                self.args.file_size_kb
                if self.args.file_size_kb > 0
                else random.choice(DEFAULT_SUITABLE_FILE_SIZES)
            )
            ...
            if any(test.errors for test in self.tests):
                print(
                    "Errors encountered in tests:",
                    [(test.vbd_name, test.errors) for test in self.tests],
                )
            else:
...
def main():
    parser = argparse.ArgumentParser(
        description="Run tests on virtual block device using 'dd'."
    )
    parser.add_argument(
        "--vbd_name",
        "-vbd",
        type=str,
        default="/dev/lsvbd1",
        help="Name of the virtual block device",
    )
    parser.add_argument(
        "--num_files", "-n", type=int, default=5, help="Number of test files to create"
    )
    parser.add_argument(
        "--file_size_kb",
        "-fs",
    ...
    parser.add_argument(
        "--clear", "-c", action="store_true", help="Clear the test directory"
    )
    parser.add_argument(
        "--mode",
        "-m",
